import { error } from '@sveltejs/kit'
import { calculateReadingTime } from '$lib/utils'
import { loadPost } from '$lib/utils/postLoader'
import { encodeSlugForUrl } from '$lib/utils/slugResolver'
import type { PageLoad } from './$types'

export const load: PageLoad = async ({ params, fetch }) => {
	// Step 1: Use our focused post loader
	const result = await loadPost(params.slug);
	
	// Step 2: Handle failure with clear error
	if (!result.success) {
		console.error('Blog post loading failed:', result);
		throw error(404, `Could not find ${result.slug || params.slug}`);
	}
	
	// Step 3: Calculate reading time and word count from one render pass.
	// wordCount feeds the BlogPosting JSON-LD; it is a real signal of depth for
	// search and answer engines, so it is worth deriving here rather than guessing.
	let readingTime: number | undefined;
	let wordCount: number | undefined;
	try {
		const rendered = result.post.render();
		if (rendered?.html) {
			readingTime = calculateReadingTime(rendered.html);
			wordCount = rendered.html
				.replace(/<[^>]+>/g, ' ')
				.split(/\s+/)
				.filter(Boolean).length;
		}
	} catch (e) {
		console.warn('Could not calculate reading time:', e);
	}
	
	// Step 4: Fetch view count (non-blocking, isolated)
	const viewCountPromise = fetch(`/api/blog-analytics?slug=${encodeSlugForUrl(result.slug)}`)
		.then(res => res.json())
		.then(data => data.viewCount || 0)
		.catch((e) => {
			console.warn('Analytics fetch failed:', e);
			return 0;
		});

	// Step 5: Return clean result. `viewCount` is nested under `streamed` so
	// SvelteKit streams it instead of blocking the page render on the analytics
	// fetch (top-level promises are no longer auto-awaited in SvelteKit 2.0).
	return {
		content: result.post,
		meta: {
			...result.metadata,
			slug: result.slug,
			readingTime,
			wordCount
		},
		streamed: { viewCount: viewCountPromise }
	};
}
