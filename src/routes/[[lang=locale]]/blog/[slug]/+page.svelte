<script lang="ts">
  ...
  import { blogPostingSchema, breadcrumbSchema } from "$lib/utils/schema";
  import { localizedPath, ogImageUrl, postPath } from "$lib/utils/seo";
...
<!-- SEO. Post bodies are the same English text under every locale, so a post
     has exactly one indexable URL: alternates are suppressed and the canonical
     always points at the unprefixed path. -->
<Seo
  title={data.meta.title}
  description={data.meta.description ?? $t("blog.metaDescription")}
  canonicalPath={postPath(data.meta.slug)}
  type="article"
  image={data.meta.img ??
    ogImageUrl({
      title: data.meta.title,
  ...
  modifiedTime={data.meta.updated ?? data.meta.date}
...
<JsonLd
  data={breadcrumbSchema([
    ...
    { name: data.meta.title, path: postPath(data.meta.slug) },
  ])}
/>
