<script lang="ts">
  ...
  import { collectionPageSchema, postListItem } from "$lib/utils/schema";
  ...
  // The 20 most recent posts, as an ItemList. Enough for a crawler to see the
  // shape of the archive without inlining 62 entries into every page load.
  $: blogSchema = collectionPageSchema({
    type: "Blog",
    name: $t("blog.pageTitle"),
    description: $t("blog.metaDescription"),
    path: "/blog",
    items: posts.slice(0, 20).map((post) => postListItem(post.slug, post)),
  });
</script>
